...
static t_bool	plane_data_to_attributes(char **data, t_minirt *minirt,
					t_ray_object *obj)
{
	if (create_translate_matrix(data, minirt, 1,
			obj->translate_mtx) == false)
		return (false);
	if (create_rotate_matrix(data, minirt, 2, obj) == false)
		return (false);
	if (create_color(data, minirt, 3, &obj->mat.color) == false)
		return (false);
...
t_bool	store_plane(t_minirt *minirt, char *line, char **data)
{
	t_ray_object	*obj;
	t_list			*new_obj;
	...
	obj = init_ray_object(0, intersect_plane, plane_normal_tremo);
	if (!obj)
		return (free_splitted_return_false((void **)data));
	new_obj = ft_lstnew(obj);
	if (plane_data_to_attributes(data, minirt, obj) == false || !new_obj)
	{
		destroy_ray_obj(obj);
		return (free_splitted_return_false((void **)data));
	}
